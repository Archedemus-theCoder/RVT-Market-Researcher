...
```
japan/
├── app_japan.py          # 일본 대시보드 (6세그먼트)
└── agents/
    ├── researcher_jp.py  # 일본 리서처 에이전트
    └── critic_jp.py      # 일본 크리틱 에이전트
data/jp/
├── sources.json          # 일본 원본 데이터
└── validated.json        # 일본 검증 데이터
```

### 일본 6세그먼트
| 세그먼트 | 대상 | 모수 |
|----------|------|------|
| S1. 신축 맨션 | 3대 도시권 신규 분양 | 도쿄+오사카+나고야 |
| S2. 리모델링 ★ | 리노베이션 맨션 | 전국 건수 × 도시권 집중도 |
| S3. 호텔/료칸 | 신규 호텔 + 료칸 리뉴얼 | 호텔수×객실 + 료칸 |
| S4. 이사수요 | 중첩 제거 후 순수 이사 | 이사건수 - S1 - S2풀 - S5 - S6 |
| S5. 기업사택 | B2B 사택/기숙사 | 기업수 × 세대수 × 성공률 |
| S6. 고령자주거 | 실버타운/서비스드 레지던스 | 시설수 × 세대수 × 집중도 |
...
# 일본 데이터
python japan/agents/researcher_jp.py
python japan/agents/critic_jp.py
